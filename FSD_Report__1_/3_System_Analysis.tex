...
\begin{itemize}
    ...
    \item \textbf{User Login (\texttt{POST /api/auth/login}):} Validates credentials by looking up the user by email and comparing the submitted password against the stored bcrypt hash using the \texttt{comparePassword} instance method on the User model. The response includes the user's role, automatically redirecting Admins to their respective dashboard.
    \item \textbf{JWT Token Generation:} Upon successful authentication, a JWT is generated encoding the user's ID, name, email, and role with a 7-day expiration. This token is stored client-side and included in the \texttt{Authorization} header of subsequent API requests.
    \item \textbf{Authentication Middleware:} Two middleware functions exist: a standard \texttt{auth} middleware that verifies the JWT for protected user routes, and an \texttt{adminAuth} middleware that strictly verifies the \texttt{admin} role to protect administrative endpoints.
\end{itemize}
...
An administrator acts as the highest-tier user with specialized access to manage the platform. This actor shares the privileges of a registered user but is granted additional oversight capabilities via the Admin Dashboard:
\begin{itemize}
    \item \textbf{Global User Management:} Can view a complete list of all registered users on the platform, including their names, emails, roles, and join dates.
    \item \textbf{Global Route Analytics:} Can view a comprehensive list of all routes created across the platform, tracking total distance, stops, and their respective creators.
    \item \textbf{Access Control:} Has exclusive access to the highly secure \texttt{/api/admin/*} endpoints, protected by specialized \texttt{adminAuth} middleware.
\end{itemize}
...
\begin{itemize}
    ...
    \item Cannot access the trip planning dashboard, Admin Dashboard, or any route-related functionality.
